...
import checklistCreate from "@/projects/tenant/modules/tools/components/ChecklistCreate.vue";
import checklistUpdate from "@/projects/tenant/modules/tools/components/ChecklistUpdate.vue";
...
const getChecklist = async () => {
  ...
  try {
    const res = await ToolServices.getChecklist();
    checklist.value = res;
  } catch (error) {
    console.log(error);
  }
...
const activeToggle = async (item: any) => {
  ...
  try {
    await ToolServices.updateChecklist(item.id, item);
...
const deleteChecklist = async (id: number) => {
  isLoading.value = true;
  try {
    await ToolServices.deleteChecklist(id);
    getChecklist();
  } catch (error) {
    console.log(error);
  }
