<template>
  <el-drawer v-model="dialogRef" :title="$t('action.edit')" size="80%">
    <div v-loading="isLoading">
      ...
      <el-tabs type="card" v-model="selectedLanguage">
...
const promotionListId = ref(0);
const s3BucketPrefix = ref("https://mm-s3-pro.s3.ap-southeast-1.amazonaws.com");
...
const submit = async () => {
  ...
  try {
    for (const lang in data.value.contents) {
      ...
      if (
        uploadImageRefs.value[lang] &&
        !langData.coverImage.includes(s3BucketPrefix.value)
      ) {
      ...
      if (
        uploadPdfRefs.value[lang] &&
        !langData.pdfUrl.includes(s3BucketPrefix.value)
      ) {
    ...
    await SystemService.updatePromotionListById(
      promotionListId.value,
      finalData
    );
    emits("submit");
...
const fecthData = async () => {
  ...
  try {
    const res = await SystemService.queryPromotionListById(
      promotionListId.value
    )
      .then((res) => {
        data.value.contents = res.data.contents;
        formData.from = res.data.from;
        formData.to = res.data.to;
      })
      .then(() => {
        fetchImages(data.value.contents);
      })
      .then(() => {
...
const fetchImages = async (contents) => {
  for (const lang in contents) {
    selectedLanguage.value = lang;
    const langData = contents[lang];
    if (langData.coverImage) {
      uploadImageRefs.value[lang].imageUrl = langData.coverImage;
    }

    if (langData.pdfUrl) {
      uploadPdfRefs.value[lang].imageUrl = langData.pdfUrl;
      uploadPdfRefs.value[lang].type = "application/pdf";
    }
  }
};
