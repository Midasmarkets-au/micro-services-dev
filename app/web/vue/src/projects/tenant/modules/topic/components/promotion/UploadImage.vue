<template>
  <div v-loading="uploading">
    ...
    >
      <div
        v-if="props.accept == 'application/pdf'"
...
import { Delete, Plus, ZoomIn, Document } from "@element-plus/icons-vue";
...
const uploadImage = async () => {
  ...
  try {
    const res = await SystemService.uploadPromotionFile(formData);
    returnUrl.value = res.link;
  } catch (error) {
...
const beforeAvatarUpload: UploadProps["beforeUpload"] = (rawFile) => {
  ...
  if (!acceptedTypes.includes(rawFile.type)) {
    fileList.value = [];
    ElNotification.error(`File must be in ${props.accept} format!`);
    return false;
  } else if (rawFile.size / 1024 / 1024 > 10) {
    fileList.value = [];
    ElNotification.error("File size cannot exceed 10MB!");
    return false;
  }
  ...
  if (rawFile) {
    ...
    emit("image-uploaded", imageUrl.value); // Emit when image is set
  } else {
