<template>
  <div style="width: 392px">
    <a
      @click="handleMouseEnter(selectedMedia)"
      class="d-block overlay mb-4"
      data-fslightbox="lightbox-hot-sales"
      href="#"
    >
      <div
        class="overlay-wrapper card-rounded min-h-225px overflow-hidden h-275px border border-1"
      >
        <template v-if="selectedMedia">
          <div
            v-if="
              [
                FileFormatTypes.JPEG,
                FileFormatTypes.JPG,
                FileFormatTypes.PNG,
              ].includes(contentType as any)
            "
            class="w-100 overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover card-rounded h-275px"
            :style="
              contentUrlWithToken !== ''
                ? `background-image: url(${contentUrlWithToken});`
                : ''
            "
          ></div>

          <div
            v-else-if="
              [FileFormatTypes.PDF].includes(contentType  as any) &&
              contentUrlWithToken !== ''
            "
          >
            <VuePdfEmbed :source="contentUrlWithToken" class="" />
          </div>
        </template>

        <div
          class="w-100 h-100 d-flex justify-content-center align-items-center bg-gray-200"
          v-else
        >
          <h1
            class="text-black-50 opacity-20 fs-1"
            style="transform: rotate(45deg)"
          >
            User has not uploaded this file yet
          </h1>
        </div>
      </div>
      <div class="overlay-layer bg-dark card-rounded bg-opacity-25">
        <i class="bi bi-eye-fill fs-2x text-white"></i>
      </div>
    </a>

    <div class="m-0 px-1">
      <div class="d-flex justify-content-between align-items-center">
        <div>
          <a
            href="#"
            class="fs-4 text-dark fw-bold text-hover-primary text-dark lh-base me-1"
            >{{ title }}</a
          >

          <el-select
            class="w-125px ms-5"
            v-model="selectedId"
            @change="handleSelectMedia(selectedId)"
          >
            <el-option
              v-if="props.medias?.length === 0"
              label="-- none --"
              :value="0"
            />
            <el-option
              v-for="(option, idx) in props.medias"
              :key="idx"
              :label="option.fileName as string"
              :value="idx"
            />
          </el-select>
        </div>

        <div class="d-flex align-items-center">
          <el-button
            v-if="selectedMedia?.status === 0"
            @click="handleApproveDocument"
            type="success"
            :icon="Check as any"
            circle
          />

          <el-button
            v-if="selectedMedia?.status === 0"
            @click="handleRejectDocument"
            type="danger"
            :icon="Close as any"
            circle
          />

          <el-button
            v-if="props.enableDelete ?? false"
            @click="deleteFile"
            type="warning"
            :icon="Delete as any"
            circle
          />

          <div
            class="text-primary me-4 fw-bold fs-3"
            v-if="selectedMedia?.status === 2"
          >
            Approved
          </div>
          <div
            class="text-danger me-4 fw-bold fs-3"
            v-if="selectedMedia?.status === 3"
          >
            Rejected
          </div>
        </div>
      </div>

      <div class="fw-semibold fs-5 text-gray-600 text-dark mt-3 d-flex">
        <template v-if="selectedMedia?.status === 3">
          <h3 class="text-danger me-4">Reason:</h3>
          <p class="">{{ selectedMedia.rejectedReason }}</p>
        </template>
      </div>
      <div class="mb-5">
        <p
          class="fs-4 text-dark fw-bold text-hover-primary text-dark lh-base me-1"
        >
          {{ context }}
        </p>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import VuePdfEmbed from "vue-pdf-embed";
import MsgPrompt from "@/core/plugins/MsgPrompt";
import { inject, onMounted, Ref, ref, watch } from "vue";
import {
  FileFormatTypes,
  getContentTypeFromMedia,
} from "@/core/types/FileFormatTypes";
import { getImageUrl } from "@/core/plugins/ProcessImageLink";
import { Check, Close, Delete } from "@element-plus/icons-vue";
import InjectKeys from "@/core/types/TenantGlobalInjectionKeys";
import TenantGlobalService from "@/projects/tenant/services/TenantGlobalService";
import UserService from "@/projects/tenant/modules/users/services/UserService";

const props = defineProps<{
  title?: string;
  descp?: string;
  medias: any;
  enableDelete?: boolean;
  verificationDetails?: any;
  context?: string;
}>();

const emits = defineEmits<{
  (e: "fileDeleted"): void;
}>();

const selectedMedia = ref(props.medias[0] ?? {});
const contentType = ref(FileFormatTypes.JPEG);
const selectedId = ref(0);
const unApprovedDocuments = ref(0);
const handleSelectMedia = (idx: number) => {
  selectedMedia.value = props.medias[idx] ?? null;
  contentType.value = getContentTypeFromMedia(selectedMedia.value);
  if (selectedMedia.value) getContentUrlWithToken();
};

watch(
  () => props.medias,
  () => handleSelectMedia(0)
);

const contentUrlWithToken = ref("");
const getContentUrlWithToken = async () => {
  if (!selectedMedia.value) return;

  contentUrlWithToken.value = await TenantGlobalService.getImageUrlWithToken(
    getImageUrl(selectedMedia.value?.guid)
  );
};

const openConfirmModal = inject(InjectKeys.OPEN_CONFIRM_MODAL);

const openRejectModal = inject(
  InjectKeys.OPEN_REJECT_REASON_MODAL,
  (formData: any) => MsgPrompt.error("Get Modal Failed")
);

const handleRejectDocument = () => {
  openRejectModal((formData: any) =>
    UserService.rejectUserDocument(
      props.verificationDetails.id,
      selectedMedia.value.id,
      formData
    )
      .then(() => MsgPrompt.success("Document Rejected Successfully"))
      .then(() => emits("fileDeleted"))
  );
};

const fileShowRef = inject<Ref>(InjectKeys.FILE_SHOW_REF);

const handleMouseEnter = async (media) => {
  if (!media) return;
  fileShowRef?.value?.show(media);
};

const handleApproveDocument = () => {
  openConfirmModal?.(
    () =>
      UserService.approveUserDocument(
        props.verificationDetails.id,
        selectedMedia.value.id
      )
        .then(() => MsgPrompt.success("Document Approved Successfully"))
        .then(() => emits("fileDeleted")),
    undefined,
    {
      confirmText: "Confirm to approve this document?",
      confirmColor: "success",
    }
  );
};

const deleteFile = () => {
  openConfirmModal?.(
    () =>
      UserService.deleteVerificationDocForUser(
        props.verificationDetails.id,
        selectedMedia.value.id
      ).then(() => emits("fileDeleted")),
    undefined,
    {
      confirmText: "Confirm to delete this document?",
      confirmColor: "warning",
    }
  );
};

onMounted(async () => {
  unApprovedDocuments.value = props.medias.filter(
    (media) => media.status === 0
  ).length;
  handleSelectMedia(0);
});
</script>
