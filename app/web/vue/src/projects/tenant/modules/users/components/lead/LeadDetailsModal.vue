...
const show = async (_lead) => {
  ...
  isArchived.value = _lead.isArchived;
...
const openCommentModal = inject(
  TenantGlobalInjectionKeys.OPEN_ADD_COMMENT_MODAL
);

const openAddedCommentModal = () => {
  openCommentModal?.((cmt: string) =>
    UserService.addCommentToLead(selectedLead.value.id, {
      content: cmt,
    }).then(getLeadDetails)
  );
};
...
const getLeadDetails = async () => {
  try {
    isLoading.value = true;
    selectedLead.value = await UserService.getLeadDetails(
...
const openRemoveConfirmModal = (account) => {
  openConfirmModal?.(
    () =>
      UserService.removeAssignedSalesAccount(
        selectedLead.value.id,
        account.uid
      ).then(() => {
        getLeadDetails();
...
const openArchiveConfirmModal = () => {
  openConfirmModal?.(
    () =>
      UserService.archiveLead(selectedLead.value.id).then(() => {
        getLeadDetails();
        emits("update");
        modalRef.value?.hide();
      }),
