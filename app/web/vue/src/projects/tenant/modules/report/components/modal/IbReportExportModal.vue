    ...
    >
      ...
      <div class="w-100 px-10">
        ...
        <el-form label-width="auto" class="border">
          ...
          <el-form-item label="IB Uids">
            <el-input
              v-model="formData.ibAccountUids"
              placeholder="IB Uids"
              type="textarea"
              clearable
              :disabled="exporting"
            ...
            <div
              v-if="showIbError"
              style="width: 100%; text-align: right; color: #900000"
            >
              Must has IB Uids / 必填
            </div>
          </el-form-item>
...
const formData = ref({
  ...
  to: null,
  ibAccountUids: "",
  checkedColumn: ["Date", "Email", "IB Account", "Group"],
} as any);
...
const submit = async () => {
  ...
  if (
    formData.value.ibAccountUids.length == 0 ||
    formData.value.ibAccountUids == null
  ) {
    showIbError.value = true;
    exporting.value = false;
    return;
  }

  showIbError.value = false;

  await ReportService.createIbReport(formData.value).then(
    MsgPrompt.success(
...
const resetFormData = () => {
  ...
  formData.value.ibAccountUids = "";
  formData.value.checkedColumn = ["Date", "Email", "IB Account", "Group"];
};
