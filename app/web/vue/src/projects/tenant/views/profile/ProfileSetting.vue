<template>
    ...
    <div class="card shadow-sm">
      ...
      <div class="card-body">
        <div :class="{ 'pe-10': !isMobile }">
          <div class="row mb-6">
            <div
              class="col-lg-4 mb-5 d-flex flex-column align-items-center justify-content-center"
            >
              <div
            ...
            <div class="col-lg-8">
              <div class="row">
                ...
                <div class="col-lg-6">
                  <label for="" class="mb-3">{{ $t("title.2fa") }}</label>
                  <div class="d-flex align-items-center gap-4">
                    <el-switch
                      @change="
                        onTwoFactorEnabledChanged(collectData.twoFactorEnabled)
                      "
                      v-model="collectData.twoFactorEnabled"
                      size="large"
                      inline-prompt
                      active-text="Open"
                      inactive-text="Close"
                    />
                    <el-button @click="showQrCodeModal">
                      {{ $t("action.view2FaQrCode") }}
                    </el-button>
                  </div>
                </div>
  ...
  <PasswordSetting />
  <QrCodeFor2FaModal ref="qrCodeFor2FaModal" />

  <TwoFaClosedVerificationCodeModal
    ref="twoFaClosedVerificationCodeModal"
    @code-sent="(val) => (collectData.twoFactorEnabled = val)"
  />
</template>
...
import QrCodeFor2FaModal from "@/components/modal/QrCodeFor2FaModal.vue";
import TenantGlobalService from "@/projects/tenant/services/TenantGlobalService";
import TwoFaClosedVerificationCodeModal from "@/components/modal/TwoFaClosedVerificationCodeModal.vue";
...
const qrCodeFor2FaModal = ref<any>();
const twoFaClosedVerificationCodeModal = ref<any>();
const showQrCodeModal = async () => {
  const res = await TenantGlobalService.getAuthenticationUri();
  qrCodeFor2FaModal.value.show(res.authenticatorUri);
};

const onTwoFactorEnabledChanged = async (value: boolean) => {
  if (value) {
    await TenantGlobalService.enable2Fa();
    await showQrCodeModal();
    return;
  } else {
    // await TenantGlobalService.disable2Fa();
    collectData.value.twoFactorEnabled = true;
    twoFaClosedVerificationCodeModal.value.show();
  }
};
