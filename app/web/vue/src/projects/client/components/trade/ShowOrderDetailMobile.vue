<template>
  <el-dialog v-model="dialogRef" :title="trade.symbol" width="500" align-center>
    ...
    <el-card>
      <div class="d-flex justify-content-between" v-if="!isCustomer">
        <div>
          {{ $t("fields.accountNumber") }}
        </div>
        <div>
          {{ trade.accountNumber }}
          <TinyCopyBox :val="trade.accountNumber.toString()"></TinyCopyBox>
        </div>
      </div>
...
onMounted(() => {
  if (isCustomer.value === undefined || isCustomer.value === null) {
