...
const defaultSalesAccount = ref(
  store.state.AuthModule.user?.defaultSalesAccount
);
...
const updateDefaultAccount = async (item: any) => {
  ...
  try {
    await AccountService.updateDefaultSalesAccount(item.uid).then(() => {
      ...
      defaultSalesAccount.value = item.uid;
      emits("update");
    });
  } catch (error) {
    ElNotification({
...
const show = (_accounts: any) => {
  ...
  if (_accounts.length > 1) {
    accounts.value = _accounts.map((account: any) => ({
      ...account,
      isDefault: defaultSalesAccount.value == account.uid,
    }));
  } else {
