  ...
  <div class="card my-5">
    <div class="card-body">
      ...
      <el-form-item
        ...
        class="mb-5"
      >
        <el-radio-group v-model="formData.accountRole" :disabled="isSubmitting">
          <el-radio
      ...
      >
        ...
        >
          <el-checkbox
            v-for="item in jpInvestorDistinction"
            :key="item.value"
            :label="item.value"
          >
            {{ item.label }}
      ...
      <div class="row pb-2">
        <div class="col-lg-6 pb-2">
          <el-form-item
            ...
            prop="accountNumber"
          >
            <el-input v-model="formData.accountNumber" :disabled="isSubmitting">
            </el-input>
          </el-form-item>
        </div>
        <div class="col-lg-6 pb-2">
...
function coverToJpLanguageForReactiveObject(objectToTranslate) {
  const originalData = objectToTranslate;
...
const jpInvestorDistinction = reactive(
  coverToJpLanguageForReactiveObject(investorDistinctionSelection)
);

watch(
  () => formData.value.accountTypes,
  (newAccountTypes) => {
    if (newAccountTypes.includes(accountTypes.productTrading)) {
      // Add special investor option if not already present
      console.log(jpInvestorDistinction);
      if (!jpInvestorDistinction.some((option) => option.value === 2)) {
        jpInvestorDistinction.push({
      ...
      // Remove special investor option if cfdTrading is not selected
      const index = jpInvestorDistinction.findIndex(
        (option) => option.value === 2
      );
      if (index !== -1) {
        jpInvestorDistinction.splice(index, 1);
      }

      // Reset investor distinction if special investor is selected
      if (formData.value.investorDistinction === 2) {
        formData.value.investorDistinction = null;
      }
    }
  }
);
...
onMounted(() => {
  ...
  if (formData.value.accountTypes !== 2) {
    const index = jpInvestorDistinction.findIndex(
      (option) => option.value === 2
    );
    if (index !== -1) {
