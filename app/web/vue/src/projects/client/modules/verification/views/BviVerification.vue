<template>
  ...
  <template v-else>
    ...
    >
      ...
      <VAgreement v-else-if="currentStepIndex === 2" />
      <VDocument v-else-if="currentStepIndex === 3" ref="vDocRef" />
...
import StepDiplay from "../components/bvi/StepDiplay.vue";
import VStarted from "../components/bvi/VerificationStarted.vue";
import VInfo from "../components/bvi/VerificationInfo.vue";
import VFinancial from "../components/bvi/VerificationFinancial.vue";
import VDocument from "../components/bvi/VerificationDocument.vue";
import VAgreement from "../components/bvi/VerificationAgreement.vue";
...
const showIbDocuments = ref(false);
const tenant = getUserTenancy();
...
provide("showIbDocuments", showIbDocuments);
...
const handleFinancial = async () => {
  if (
    (formData.value.bg1 == 1 || formData.value.bg2 == 1) &&
    config.value.verificationQuizEnabled
  ) {
...
const handleAgreement = async () => {
  if (tenant === tenancies.sea && formData.value.consent_2 === false) {
    return Promise.reject(t("error.electronicIdentityVerificationRequired"));
  }
...
const checkIfIb = (referInfo) => {
  if (
    referInfo.data.serviceType === AccountRoleTypes.IB ||
    referInfo.data.serviceType === AccountRoleTypes.Broker
  ) {
    showIbDocuments.value = true;
  } else {
    showIbDocuments.value = false;
  }
  console.log("showIbDocuments.value", showIbDocuments.value);
};
  ...
  consent_2: [
    { required: true, message: t("error.fieldIsRequired"), trigger: "change" },
    {
      validator: async (_: any, value: boolean) => {
        ...
        if (tenant === tenancies.sea && value === false) {
          return Promise.reject(
            t("error.electronicIdentityVerificationRequired")
          );
        }
