...
const item = ref<any>(items?.value?.data?.quiz || []);
...
const submitForm = async () => {
  ...
  try {
    ...
    MsgPrompt.success(t("tip.testPass"), "Your Score: " + score.value + "/10");
  } catch (error) {
...
const prepareData = () => {
  ...
  const keys = item.value.map((obj) => obj.id);
