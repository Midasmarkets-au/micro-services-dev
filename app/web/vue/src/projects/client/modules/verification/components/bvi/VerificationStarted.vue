...
const { items, formData, isSubmitting, accountTypeSelections } =
  useVerificationForm();
...
const setAccountType = () => {
  //handle account type default to vn for vn site
  if (
    accountTypeSelections?.value?.some((item) => item.value === AccountTypes.Vn)
  )
    return AccountTypes.Vn;

  return accountTypeSelections?.value[0]?.value;
};

onMounted(() => {
  initializeFormData(item.value, formData.value, {
    currency: getDefaultCurrency(840, ConfigCurrencySelections.value), // USD
    accountType: setAccountType(),
    leverage: getMaxLeverage(ConfigLeverageSelections.value),
    serviceId: getDefaultServiceId(
      hasMt5.value,
      ConfigRealServiceSelections.value
    ),
    questions: createDefaultQuestions(true), // BVI 默认为 true
  });
});
</script>
