...
export function CreateDemoAccountModal({
        ...
        try {
          ...
          if (result.success && result.data) {
            ...
            if (result.data.tradingPlatformAvailable?.length > 0) {
              ...
              setPlatform(String(first.platform));
            }
  ...
  const handleSubmit = async () => {
    if (!platform || !accountType || !currency || !leverage || !amount) return;

    const params: CreateDemoAccountParams = {
    ...
    const result = await execute(createDemoAccount, params);
    if (result.success) {
  ...
  const platformOptions = config?.tradingPlatformAvailable?.map((item) => {
    const name = serviceMap[item.serviceId]?.serverName || getPlatformName(item.platform);
    const shortLabel = serviceMap[item.serviceId]?.platformName || getPlatformName(item.platform);
    return {
      value: String(item.platform),
      label: name,
      shortLabel: shortLabel.includes('5') ? 'MT5' : 'MT4',
    };
  }) || [];
  ...
  const selectedPlatformLabel = platformOptions.find(o => o.value === platform)?.label || '-';
  ...
  const canProceed = !!platform && !!accountType && !!currency && !!leverage && !!amount;
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) handleClose(); else onOpenChange(v); }}>
      ...
      >
        <div className="flex flex-col gap-6">
          <DialogHeader>
            <DialogTitle>{t('modal.createDemoAccount')}</DialogTitle>
          </DialogHeader>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  placeholder={t('placeholder.selectAmount')}
                />
              </div>
            </div>
          )}
