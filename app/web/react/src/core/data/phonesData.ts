export const getRegionCodes = (
  // preferredCountries = ["au", "cn", "hk", "tw", "vn", "jp", "mn"]
  preferredCountries = ["cn", "hk", "tw", "vn", "jp"]
) => {
  // console.log("preferredCountries", preferredCountries);
  const regionCodeList: Record<string, typeof phonesData[keyof typeof phonesData]> = {};
  preferredCountries.forEach((item) => {
    regionCodeList[item] = phonesData[item as keyof typeof phonesData];
  });

  Object.keys(phonesData).forEach((countryCode: string) => {
    if (!preferredCountries.includes(countryCode)) {
      regionCodeList[countryCode] = phonesData[countryCode as keyof typeof phonesData];
    }
  });
...
export const getDataByCode = (code: string) => {
  return phonesData[code as keyof typeof phonesData] || {};
};
