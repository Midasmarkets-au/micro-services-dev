...
export default function RepTransactionPage() {
  ...
  const defaultFilterParam = useMemo(() => {
    const param: Record<string, unknown> = { pageSize: 25 };
    if (accountFromQuery) {
      param.account = accountFromQuery;
      setTimeout(() => router.replace('/rep/transaction'), 3000);
    }
    return param;
  ...
  const fetchData = useCallback(
    async (p: number, extraParams?: Record<string, unknown>) => {
      ...
      try {
        ...
        const result = await executeRef.current(async () => fetchAction<SalesTransactionListResponse>('getRepTransactionReports', repAccount.uid, {
          page: p,
          size: pageSize,
    ...
    [repAccount, filterParams, isClient, pageSize],
  );

  const repUid = repAccount?.uid;
  useEffect(() => {
    if (repUid) fetchData(1);
    else setIsLoading(false);
  }, [repUid, isClient]); // eslint-disable-line react-hooks/exhaustive-deps
