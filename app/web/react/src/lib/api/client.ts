...
/**
 * 判断字符串是否符合后端错误码命名规范，用作 errorCode 提取。
 * 命中以下两类（任一）即视为错误码：
 *   1. __XXX__：双下划线包裹的全大写错误码（如 __PENDING_TRANSFER_EXIST__）
 *   2. SCREAMING_SNAKE / Mixed_Snake：以字母开头、含至少一个下划线分隔（如 CODE_ALREADY_SENT、Too_many_pending_deposits）
 * 不命中：单词形态（如 "Unauthorized"、"BadRequest"、"Wallet address already exists"）。
 */
