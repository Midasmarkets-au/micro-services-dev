...
) : TenantBaseControllerV2
{
    ...
    public async Task<IActionResult> TodayActiveUser()
    {
        ...
        var results = rawInfos
            ...
            {
                var raw = x.ToString()!;
                // Format: {email}_{partyId}_{tenantId}_{datetime}
                // Split from the right to handle emails that contain underscores.
                var lastUnderscore = raw.LastIndexOf('_');
                var dateStr = raw[(lastUnderscore + 1)..];
                var rest = raw[..lastUnderscore];

                var secondLast = rest.LastIndexOf('_');
                var tenantId = rest[(secondLast + 1)..];
                rest = rest[..secondLast];

                var thirdLast = rest.LastIndexOf('_');
                var partyId = rest[(thirdLast + 1)..];
                var email = rest[..thirdLast];

                return new { email, partyId, tenantId, dateStr };
            })
            ...
            .Select(x => new
            {
                ...
                Users = x.Select(y => new
                {
                    PartyId = y.partyId,
                    Email = y.email,
                    LastRequest = $"{(DateTime.UtcNow - Utils.ParseToUTC(y.dateStr)).TotalSeconds}s"
                })
            })
            .ToList();
