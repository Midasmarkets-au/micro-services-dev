using OpenIddict.Validation.AspNetCore;
using Bacera.Gateway.Interfaces;
using Bacera.Gateway.ViewModels.Tenant;
using Microsoft.AspNetCore.Authentication.JwtBearer;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore;
using Newtonsoft.Json;

namespace Bacera.Gateway.Web.Areas.Tenant.Controllers;

using M = Verification;

[Authorize(AuthenticationSchemes = OpenIddictValidationAspNetCoreDefaults.AuthenticationScheme)]
[Tags("Tenant/KYC Form")]
public class KycController : TenantBaseController
{
    private readonly AuthDbContext _authDbContext;
    private readonly TenantDbContext _tenantDbContext;

    public KycController(TenantDbContext tenantDbContext, AuthDbContext authDbContext)
    {
        _authDbContext = authDbContext;
        _tenantDbContext = tenantDbContext;
    }

    /// <summary>
    /// KYC form pagination
    /// </summary>
    /// <param name="criteria"></param>
    /// <returns></returns>
    [HttpGet]
    [ProducesResponseType(typeof(Result<List<VerificationViewModel>, Verification.Criteria>), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Index([FromQuery] Verification.Criteria? criteria)
    {
        criteria ??= new Verification.Criteria();
        criteria.Type = VerificationTypes.KycForm;
        var hideEmail = ShouldHideEmail();
        var items = await _tenantDbContext.Verifications
            .PagedFilterBy(criteria)
            .ToTenantViewModel(hideEmail)
            .ToListAsync();

        // await FulfillUserBasicInfo(_authDbContext, items);

        return Ok(Result<List<VerificationViewModel>, Verification.Criteria>.Of(items, criteria));
    }

    /// <summary>
    /// Get KYC form by party Id
    /// </summary>
    /// <param name="partyId"></param>
    /// <returns></returns>
    [HttpGet("{partyId:long}")]
    [ProducesResponseType(typeof(M), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Get(long partyId)
    {
        var item = await GetVerificationWithItem(partyId);
        if (item == null) return NotFound();
        return Ok(item);
    }

    /// <summary>
    /// Update KYC form by party Id
    /// </summary>
    /// <param name="partyId"></param>
    /// <param name="spec"></param>
    /// <returns></returns>
    [HttpPut("{partyId:long}")]
    [ProducesResponseType(typeof(M), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Update(long partyId, [FromBody] KycFormViewModel spec)
    {
        spec.StaffPartyId = GetPartyId();
        var item = await GetVerificationWithItem(partyId);
        if (item == null)
            return NotFound();

        var vItem = item.VerificationItems.FirstOrDefault();
        if (vItem == null) return NotFound();

        vItem.Content = Utils.JsonSerializeObject(spec);
        vItem.UpdatedOn = DateTime.UtcNow;

        item.UpdatedOn = DateTime.UtcNow;
        item.Status = (int)VerificationStatusTypes.UnderReview;
        _tenantDbContext.Verifications.Update(item);
        await _tenantDbContext.SaveChangesAsync();

        return Ok(item);
    }

    /// <summary>
    /// Create KYC form by party Id
    /// </summary>
    /// <param name="partyId"></param>
    /// <param name="spec"></param>
    /// <returns></returns>
    [HttpPost("{partyId:long}")]
    [ProducesResponseType(typeof(M), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Create(long partyId, [FromBody] KycFormViewModel spec)
    {
        var item = await GetVerificationWithItem(partyId);
        spec.StaffPartyId = GetPartyId();
        if (item != null)
            return BadRequest(Result.Error(ResultMessage.Common.RecordExists));

        var vItem = new VerificationItem
        {
            Content = Utils.JsonSerializeObject(spec),
            Category = VerificationCategoryTypes.KycForm,
        };
        var form = new Verification
        {
            PartyId = partyId,
            Type = (short)VerificationTypes.KycForm,
            Status = (int)VerificationStatusTypes.AwaitingReview,
            Note = string.Empty,
            VerificationItems = new List<VerificationItem> { vItem },
        };
        await _tenantDbContext.Verifications.AddAsync(form);
        await _tenantDbContext.SaveChangesAsync();

        return Ok(form);
    }


    /// <summary>
    /// Change KYC form status to AwaitingReview by party Id
    /// </summary>
    /// <param name="partyId"></param>
    /// <returns></returns>
    [HttpPut("{partyId:long}/awaiting-review")]
    [ProducesResponseType(typeof(M), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> AwaitingReview(long partyId)
    {
        var item = await _tenantDbContext.Verifications
            .Where(x => x.Type == (int)VerificationTypes.KycForm)
            .Where(x => x.PartyId == partyId)
            .SingleOrDefaultAsync();
        if (item == null)
            return NotFound();
        item.Status = (int)VerificationStatusTypes.AwaitingReview;
        item.UpdatedOn = DateTime.UtcNow;
        _tenantDbContext.Verifications.Update(item);
        await _tenantDbContext.SaveChangesAsync();

        return NoContent();
    }

    /// <summary>
    /// Sign KYC form by party Id, transfer to AwaitingApprove status
    /// </summary>
    /// <param name="partyId"></param>
    /// <param name="spec"></param>
    /// <returns></returns>
    [HttpPut("{partyId:long}/sign")]
    [ProducesResponseType(typeof(M), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Sign(long partyId, [FromBody] KycFormViewModel spec)
    {
        var validator = new KycFormViewModelSignValidator();
        var result = await validator.ValidateAsync(spec);
        if (!result.IsValid)
            return BadRequest(Result.Error(ResultMessage.Common.InvalidInput,
                result.Errors.Select(x => x.ErrorMessage).ToList()));
        return await HandleKycForm(partyId, spec, VerificationStatusTypes.AwaitingApprove, 
            ResultMessage.Verification.CannotUnderReview);
    }

    /// <summary>
    /// Approve KYC form by party Id
    /// </summary>
    /// <param name="partyId"></param>
    /// <param name="spec"></param>
    /// <returns></returns>
    [HttpPut("{partyId:long}/finalize")]
    [ProducesResponseType(typeof(M), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Finalize(long partyId, [FromBody] KycFormViewModel spec)
    {
        long userPartyId = GetPartyId(), tenantId = GetTenantId();
        var email = await _authDbContext.Users
            .Where(x => x.TenantId == tenantId && x.PartyId == userPartyId)
            .Select(x => x.Email)
            .FirstOrDefaultAsync();
        if (email == null) return NotFound();

        // if (email != "compliance@au.thebcr.com")
        // {
        //     return BadRequest(Result.Error(ResultMessage.Common.ActionNotAllow));
        // }

        var validator = new KycFormViewModelFinalizeValidator();
        var result = await validator.ValidateAsync(spec);
        if (!result.IsValid)
            return BadRequest(Result.Error(ResultMessage.Common.InvalidInput,
                result.Errors.Select(x => x.ErrorMessage).ToList()));
        var response = await HandleKycForm(partyId, spec, VerificationStatusTypes.Approved, 
            ResultMessage.Verification.CannotApprove);

        await RecordHistory(partyId, spec); // Assuming this method's name starts with an uppercase
        return response;
    }

    /// <summary>
    /// Get KYC form history by party Id
    /// </summary>
    /// <param name="partyId"></param>
    /// <returns></returns>
    [HttpGet("{partyId:long}/history")]
    [ProducesResponseType(typeof(M), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> GetHistory(long partyId)
    {
        var supplement = await _tenantDbContext.Supplements
            .Where(x => x.Type == (int)SupplementTypes.KycFormHistory)
            .SingleOrDefaultAsync(x => x.RowId == partyId);
        if (supplement == null) return Ok(new List<KycFormViewModel>());

        var model = JsonConvert.DeserializeObject<List<KycFormViewModel>>(supplement.Data) ??
                    new List<KycFormViewModel>();
        return Ok(model);
    }

    /// <summary>
    /// Reject KYC form by party Id
    /// </summary>
    /// <param name="partyId"></param>
    /// <returns></returns>
    [HttpPut("{partyId:long}/reject")]
    [ProducesResponseType(typeof(M), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> Reject(long partyId)
    {
        var item = await _tenantDbContext.Verifications
            .Where(x => x.Type == (int)VerificationTypes.KycForm)
            .SingleOrDefaultAsync(x => x.PartyId == partyId);
        if (item == null)
            return NotFound();

        item.Status = (int)VerificationStatusTypes.Rejected;
        item.UpdatedOn = DateTime.UtcNow;

        _tenantDbContext.Verifications.Update(item);
        await _tenantDbContext.SaveChangesAsync();

        return NoContent();
    }

    /// <summary>
    /// Get Compliance Signature
    /// </summary>
    /// <returns></returns>
    [HttpGet("complianceSig")]
    [ProducesResponseType(typeof(M), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> GetComplianceSignature()
    {
        var user = await _authDbContext.Users
            .Where(x => x.PartyId == GetPartyId() && x.TenantId == GetTenantId())
            .FirstOrDefaultAsync();
        if (user == null) return NotFound();

        // Temporary solution for au check now 11/03/2025
        // if (user.Email == "compliance@au.thebcr.com")
        //     return Ok(GetVicSignature());

        // return BadRequest(Result.Error(ResultMessage.Common.ActionNotAllow));
        return Ok(GetVicSignature());
    }

    private async Task<IActionResult> HandleKycForm(long partyId, KycFormViewModel spec,
        VerificationStatusTypes newStatus, string cannotApproveMessage)
    {
        var item = await GetVerificationWithItem(partyId);
        if (item == null || item.VerificationItems.Count == 0)
            return NotFound();

        var vItem = item.VerificationItems.First();
        vItem.Content = Utils.JsonSerializeObject(spec);
        vItem.UpdatedOn = DateTime.UtcNow;

        item.Status = (int)newStatus;
        item.UpdatedOn = DateTime.UtcNow;

        _tenantDbContext.Verifications.Update(item);
        await _tenantDbContext.SaveChangesAsync();

        return Ok(spec);
    }

    private async Task RecordHistory(long partyId, KycFormViewModel model)
    {
        const int keepRecord = 100;
        var supplementHistory = await _tenantDbContext.Supplements
                                    .Where(x => x.Type == (int)SupplementTypes.KycFormHistory)
                                    .SingleOrDefaultAsync(x => x.RowId == partyId)
                                ?? Supplement.Build(SupplementTypes.KycFormHistory, partyId, "[]");
        var history = JsonConvert.DeserializeObject<List<KycFormViewModel>>(supplementHistory.Data) ??
                      new List<KycFormViewModel>();
        history.Add(model);
        history = history.TakeLast(keepRecord).ToList();

        supplementHistory.Data = Utils.JsonSerializeObject(history);
        supplementHistory.UpdatedOn = DateTime.UtcNow;
        _tenantDbContext.Supplements.Update(supplementHistory);
        await _tenantDbContext.SaveChangesAsync();
    }

    private async Task<Verification?> GetVerificationWithItem(long partyId)
    {
        var item = await _tenantDbContext.Verifications
            .Where(x => x.Type == (int)VerificationTypes.KycForm)
            .Include(x => x.VerificationItems)
            .SingleOrDefaultAsync(x => x.PartyId == partyId);
        if (item?.VerificationItems != null)
        {
            item.VerificationItems = item.VerificationItems.OrderByDescending(x => x.Id).ToList();
        }

        return item;
    }

    // private async Task FulfillUserBasicInfo<T>(AuthDbContext ctx, List<T> iHasUserItems)
    //     where T : class, ICanFulfillUserBasicInfo
    // {
    //     var partyIds = iHasUserItems
    //         .Select(x => x.PartyId)
    //         .Where(x => x > 0)
    //         .Distinct()
    //         .ToList();
    //     var tenantId = GetTenantId();
    //     var users = await ctx.Users
    //         .Where(x => x.TenantId == tenantId && partyIds.Contains(x.PartyId))
    //         .ToTenantBasicViewModel()
    //         .ToListAsync();
    //
    //     foreach (var item in iHasUserItems)
    //     {
    //         var user = users.FirstOrDefault(x => x.PartyId == item.PartyId);
    //         if (user != null) item.User = user;
    //     }
    // }

    private static string GetVicSignature()
    {
        return
            "data:image/jpeg;base64,";
    }
}