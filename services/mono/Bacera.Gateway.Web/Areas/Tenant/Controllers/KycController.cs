...
public class KycController(TenantDbContext tenantDbContext, AuthDbContext authDbContext) : TenantBaseController
{
    [HttpGet]
    ...
    public async Task<IActionResult> Update(long partyId, [FromBody] KycFormViewModel spec)
    {
        ...
        tenantDbContext.Verifications.Update(item);
    ...
    public async Task<IActionResult> Create(long partyId, [FromBody] KycFormViewModel spec)
    {
        ...
        await tenantDbContext.Verifications.AddAsync(form);
        await tenantDbContext.SaveChangesAsync();
    ...
    public async Task<IActionResult> Finalize(long partyId, [FromBody] KycFormViewModel spec)
    {
        ...
        var email = await authDbContext.Users
        ...
        await RecordHistory(partyId, spec);
        return response;
    }

    [HttpGet("{partyId:long}/history")]
    [ProducesResponseType(typeof(List<KycFormViewModel>), StatusCodes.Status200OK)]
    ...
    {
        var item = await GetVerificationWithItem(partyId);
        if (item == null || item.VerificationItems.Count == 0) return NotFound();
    ...
    private async Task RecordHistory(long partyId, KycFormViewModel model)
    {
        ...
        var supplementHistory = await tenantDbContext.Supplements
                                    .Where(x => x.Type == (int)SupplementTypes.KycFormHistory)
                                    .SingleOrDefaultAsync(x => x.RowId == partyId)
        ...
        tenantDbContext.Supplements.Update(supplementHistory);
        await tenantDbContext.SaveChangesAsync();
    }
