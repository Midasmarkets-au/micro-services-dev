...
public class CreateAccountRequest : BaseRequest, ICreateTradeAccountRequestModel
{
    ...
    public static CreateAccountRequest Build(string name, string group, int leverage, string? password = "",
        string? investorPassword = "")
        => new()
        {
            Login = 0, Name = name, Group = group, Leverage = leverage, Password = password ?? string.Empty,
            PasswordInvestor = investorPassword ?? string.Empty
        };

    // ReSharper disable once StringLiteralTypo
    public override string RequestQuery() => $"action=createaccount&request_id={Guid.NewGuid()}&URLEncode=1&" + this.ToQueryString();

    public string ToJson() => JsonConvert.SerializeObject(this);
}

public static class CreateAccountRequestExtension
{
    public static object ToMt5Spec(this CreateAccountRequest me)
    {
        // Fix double-escaping issue: PostgreSQL stores backslashes as \\ in JSON,
        // but sometimes they get double-escaped. Unescape them here to ensure
        // the group name is correct before sending to MT5 API.
        var unescapedGroup = me.Group?.Replace("\\\\", "\\") ?? string.Empty;
        
        return new
        {
            me.Login,
            Group = unescapedGroup,
            PassMain = me.Password,
            PassInvestor = me.PasswordInvestor,
            PhonePassword = me.PasswordPhone,
            me.Name,
            me.Leverage,
            me.Country,
            me.City,
